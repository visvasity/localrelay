...
func (v *Options) SetFlags(fset *flag.FlagSet, defaults *Options) {
	...
	fset.IntVar(&v.HTTPPort, "http-port", defaults.HTTPPort, "TCP Port number for the HTTP endpoint")
	fset.IntVar(&v.HTTPSPort, "https-port", defaults.HTTPSPort, "TCP Port number for the HTTPS endpoint")
	fset.StringVar(&v.TLSCertPath, "tls-cert", "", "Path to the TLS certificate file")
	fset.StringVar(&v.TLSKeyPath, "tls-key", "", "Path to the TLS private key file")
	fset.StringVar(&v.SocketsDir, "sockets-dir", "", "Path to the reverse proxy target sockets directory")
}
